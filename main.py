...
import flask, telebot, os, requests, urllib, time, random
from flask import Flask, request
from telebot import types
from datetime import date, timedelta
from random import randrange

secret = "F4c3Gb53Ur103dHn"
token = '642122532:AAGKg4s2_ffJqDNTrqvbI7-qeFRxNEOBPV8'
bot = telebot.TeleBot(token, threaded=False)

bot.remove_webhook()
time.sleep(0.1)
bot.set_webhook(url="https://iiktbot.herokuapp.com//{}".format(secret), max_connections=1)
app = Flask(__name__)
            ...
            if not any(words in message.text for words in days_tuple) and not any(words in message.text for words in weekdays_tuple):
                if message.from_user.id in first_group.values():
                    if date.today().weekday() == 0:
                        ...
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_LIGHTWEEK_1GROUP_SATURDAY, reply_to_message_id=message.message_id)
                    elif date.today().weekday() == 6:
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_LIGHTWEEK_1GROUP_SUNDAY, reply_to_message_id=message.message_id)
                    ...
                    if date.today().weekday() == 0:
                        ...
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_LIGHTWEEK_2GROUP_SATURDAY, reply_to_message_id=message.message_id)
                    elif date.today().weekday() == 6:
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_LIGHTWEEK_2GROUP_SUNDAY, reply_to_message_id=message.message_id)
            ...
            if not any(words in message.text for words in days_tuple) and not any(words in message.text for words in weekdays_tuple):
                if message.from_user.id in first_group.values():
                    if date.today().weekday() == 0:
                        ...
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_DARKWEEK_1GROUP_SATURDAY, reply_to_message_id=message.message_id)
                    elif date.today().weekday() == 6:
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_DARKWEEK_1GROUP_SUNDAY, reply_to_message_id=message.message_id)
                elif message.from_user.id in second_group.values():
                    if date.today().weekday() == 0:
                        if message.from_user.id in first_group_eng.values():
                        ...
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_DARKWEEK_2GROUP_SATURDAY, reply_to_message_id=message.message_id)
                    elif date.today().weekday() == 6:
                        bot.send_message(message.chat.id, today_template + CS18_SCHEDULE_DARKWEEK_2GROUP_SUNDAY, reply_to_message_id=message.message_id)
                elif message.from_user.id not in all_students.values():
                    bot.send_message(message.chat.id, "мы ещё не знакомы, напиши мне в личку что-нибудь", reply_to_message_id=message.message_id)
            elif "сегодня" in message.text and not any(words in message.text for words in today_tuple):
                if message.from_user.id in first_group.values():
