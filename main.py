...
def predefined_messages(message):
	...
	weekdays_all_matches = sum(x in msg for x in weekdays_all_tuple)
	exceptions_matches = sum(x in msg for x in exceptions_tuple)

	contradiction = 'no'

	if days_matches == 0 and exceptions_matches == 0:
		contradiction = 'no'
	...
	if (('когда' in msg or (('на' in msg or 'во' in msg) and 'сколько' in msg) or ('на' in msg and 'который' in msg and 'час' in msg)) and ('пары' in msg or 'пара' in msg or 'идти' in msg or 'приходить' in msg or 'в уни' in msg)) or (('на 8' in msg and 'пары' in msg and '?' in msg) or ('на 10' in msg and 'пары' in msg and '?' in msg) or ('на 11' in msg and 'пары' in msg and '?' in msg) or ('на 13' in msg and 'пары' in msg and '?' in msg) or ('на 14' in msg and 'пары' in msg and '?' in msg)):
		first_class_request_match = 'yes'
	else:
	...
	if uid in first_group.keys() or uid in second_group.keys():
		...
		if weekorder == True:
			...
			if any(words in msg for words in classes_tuple):
					...
					if time_day_beg <= time_now <= time_uni_end:
						...
						if uid in first_group.keys():
							...
							if date_weekday + 1 == 7:
								...
								bot.send_message(cid, tomorrow_template + '\n\n' + CS18_SCHEDULE_LIGHTWEEK_2GROUP_SUNDAY, reply_to_message_id=mid)
				elif 'сегодня' in msg and 0 < days_matches < 2 and exceptions_matches == 0:
					if uid in first_group.keys():
						if date_weekday == 0:
							if uid in first_group_eng.keys():
								bot.send_message(cid, today_template + '\n\n' + CS18_SCHEDULE_LIGHTWEEK_1GROUP_1SUBGROUP_MONDAY, reply_to_message_id=mid)
						...
						if date_weekday == 0:
							...
							bot.send_message(cid, today_template + '\n\n' + CS18_SCHEDULE_LIGHTWEEK_2GROUP_SUNDAY, reply_to_message_id=mid)
				elif 'вчера' in msg and 0 < days_matches < 2 and exceptions_matches == 0:
					if uid in first_group.keys():
						if date_weekday - 1 == 0:
							if uid in first_group_eng.keys():
						...
						if date_weekday - 1 == 0:
							...
							bot.send_message(cid, yesterday_template + '\n\n' + CS18_SCHEDULE_LIGHTWEEK_2GROUP_SUNDAY, reply_to_message_id=mid)
				elif 'завтра' in msg and 0 < days_matches < 2 and exceptions_matches == 0:
					if uid in first_group.keys():
						if date_weekday + 1 == 7:
							if uid in first_group_eng.keys():
								bot.send_message(cid, tomorrow_template + '\n\n' + CS18_SCHEDULE_LIGHTWEEK_1GROUP_1SUBGROUP_MONDAY, reply_to_message_id=mid)
				...
				if 0 < days_matches < 2 and weekdays_all_matches == 0 and exceptions_matches == 0:
					if time_day_beg <= time_now <= time_uni_end:
						if uid in first_group.keys():
							if date_weekday == 0:
