...
def predefined_commands(message):
	...
	if weekorder == True:
		if date.today().weekday() == 0 and "какой" in message.text and "день" in message.text:
		...
		elif "сегодня" in message.text and "вчера" not in message.text and "завтра" not in message.text:
			...
			if "пары" in message.text or "парам" in message.text or "расписание" in message.text or "расписанию" in message.text or "предметы" in message.text or "предметам" in message.text or "у нас завтра" in message.text:
				if message.from_user.id in first_group.values():
					...
					bot.send_message(message.chat.id, "вряд ли ты здесь учишься", reply_to_message_id = message.message_id)
			elif "парам на неделю" in message.text or "парам на неделе" in message.text or "расписание на неделю" in message.text or "расписание на неделе" in message.text or "расписанию на неделю" in message.text or "предметы на неделе" in message.text or "предметам на неделе" in message.text or "предметам на неделю" in message.text or "/schedule" in message.text or "/schedule@IIKTBot" in message.text:
				if message.from_user.id in first_group.values():
